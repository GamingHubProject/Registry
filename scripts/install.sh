...
# The Nginx worker only ever needs to serve static files, never write
# anything and never see application secrets. Grant it bare traversal on
# the install root, then read+traverse only on public/ (served directly)
# and storage/app/public (served through the public/storage symlink for
# user uploads) — explicitly NOT the rest of storage/, .env, or plugins/.
"${SUDO[@]}" setfacl -m "u:${NGINX_WORKER_UID}:x" "$INSTALL_DIR"
"${SUDO[@]}" mkdir -p "$INSTALL_DIR/storage/app/public"
for nginx_served_dir in "$INSTALL_DIR/public" "$INSTALL_DIR/storage/app/public"; do
    "${SUDO[@]}" setfacl -R -m "u:${NGINX_WORKER_UID}:rX" "$nginx_served_dir"
    "${SUDO[@]}" find "$nginx_served_dir" -type d -exec setfacl -m "d:u:${NGINX_WORKER_UID}:rX" {} +
done

info "Azuriom is writable by the PHP container; the Nginx worker can read only public/ and storage/app/public, not .env or storage/app/gaming-hub-manager."
...
# A real write, not `test -w`: with musl/BusyBox, `test -w` (access(2)) does
# not consistently honor POSIX ACLs on a bind mount the way open(2) does, so
# it can report "not writable" even though the ACL genuinely grants write.
# Creating and removing a real file reflects the effective access instead.
"${DOCKER[@]}" compose exec -T --user "$WWW_UID" app \
    sh -c 'touch "/var/www/azuriom/storage/.write-test.$$" && rm -f "/var/www/azuriom/storage/.write-test.$$" && touch "/var/www/azuriom/bootstrap/cache/.write-test.$$" && rm -f "/var/www/azuriom/bootstrap/cache/.write-test.$$"' \
