...
INSTALLER_VERSION="2.7-manager-release-selector"
...
PY_CURRENT
    ...
    local installed_release_tag=""
    local installed_asset_id=""
    if [[ -f "$release_marker" ]]; then
        installed_release_tag="$(awk -F= '$1 == "TAG" { sub(/^TAG=/, ""); print; exit }' "$release_marker")"
        installed_asset_id="$(awk -F= '$1 == "ASSET_ID" { sub(/^ASSET_ID=/, ""); print; exit }' "$release_marker")"
    fi

    printf '\n\033[1;35mUpgrade Gaming Hub Manager\033[0m\n'
    printf 'Installed plugin version: %s\n' "$current_version"
    if [[ -n "$installed_release_tag" ]]; then
        printf 'Installed release tag:    %s\n' "$installed_release_tag"
    fi
    ...
    local release_mode release_api requested_tag=""
    case "$release_choice" in
        1)
            release_mode="latest"
            release_api="${MANAGER_API}?nocache=$(date +%s)"
            printf 'Checking the latest stable GamingHubProject/Manager release...\n'
            ;;
        2)
            release_mode="specific"
            while true; do
                read -r -p "Release tag (example: v0.1.4m0.4): " requested_tag
                [[ -n "$requested_tag" ]] || { warn "Enter a release tag."; continue; }
                [[ "$requested_tag" != *[[:space:]]* ]] || { warn "Release tags must not contain whitespace."; continue; }
                break
            done
            local encoded_tag
...
PY_TAG
            ...
            printf 'Checking GamingHubProject/Manager release tag %s...\n' "$requested_tag"
            ;;
        3)
            ...
            ;;
        *)
            fail "Invalid release selection. Run the installer again."
            ;;
    esac
    ...
    if ! curl -fsSL --retry 3 --retry-delay 2 \
        ...
        if [[ "$release_mode" == "specific" ]]; then
            fail "Could not find Manager release tag ${requested_tag}."
        fi
        fail "Could not query the latest Gaming Hub Manager release."
    fi

    local release_info
    release_info="$("$python_bin" - "$upgrade_tmp/release.json" "$release_mode" <<'PY_RELEASE'
import json, re, sys
with open(sys.argv[1], encoding="utf-8") as f:
    release = json.load(f)
mode = sys.argv[2]
...
if mode == "latest" and release.get("prerelease"):
    raise SystemExit("latest endpoint unexpectedly returned a prerelease")
...
PY_RELEASE
        ...
        rm -rf "$upgrade_tmp"
        fail "Could not resolve exactly one packaged gaming-hub-manager-v*.zip release asset."
    }

    local latest_tag asset_api_url browser_url asset_digest asset_size asset_id is_prerelease
    IFS=$'\t' read -r latest_tag asset_api_url browser_url asset_digest asset_size asset_id is_prerelease <<< "$release_info"

    [[ -n "$latest_tag" && -n "$asset_api_url" && -n "$asset_id" ]] || {
        rm -rf "$upgrade_tmp"
        fail "The selected Manager release metadata is incomplete."
    }

    printf 'Release:           %s\n' "$latest_tag"
    printf 'Release asset ID:  %s\n' "$asset_id"
    if [[ "$is_prerelease" == "1" ]]; then
        warn "The selected tag is marked as a GitHub prerelease."
    fi
...
PY_MANIFEST
    ...
    # The GitHub release tag is a release identifier, not the plugin semantic version.
    # Milestone/build tags such as v0.1.4m0.4 may intentionally package plugin version 0.1.4.
    if [[ "$latest_tag" != "v${latest_version}" && "$latest_tag" != "$latest_version" ]]; then
        info "Release tag and plugin version differ; plugin.json is treated as authoritative."
    fi

    if [[ "$installed_release_tag" == "$latest_tag" \
    ...
    # Compatibility for installations created before release-tag tracking existed.
    # A conventional exact tag (vX.Y.Z) plus matching plugin version is safely considered current.
    if [[ -z "$installed_release_tag" \
        && "$current_version" == "$latest_version" \
        && ( "$latest_tag" == "v${latest_version}" || "$latest_tag" == "$latest_version" ) ]]; then
    ...
    if [[ "$current_version" == "$latest_version" ]]; then
        warn "The selected release uses the same plugin version (${latest_version}) but a different release tag/build."
        printf 'The selected release will still be installed.\n'
        ...
        if [[ "$highest_version" == "$current_version" ]]; then
            if [[ "$release_mode" == "specific" ]]; then
                warn "You selected an older plugin version (${latest_version}) than the installed version (${current_version})."
                warn "Continuing will perform an intentional downgrade after confirmation."
            else
...
PY_ENABLED
    ...
    local stamp backup_dir storage_dir safe_tag
    stamp="$(date +%Y%m%d-%H%M%S)"
...
PY_AFTER
    ...
    [[ "$installed_after" == "$latest_version" ]] || {
        rm -rf "$upgrade_tmp"
        fail "Upgrade verification failed: expected plugin ${latest_version}, found ${installed_after}. Backup: ${backup_dir}"
    }
...
MARKER
    if "${SUDO[@]}" cp "$marker_tmp" "$release_marker" && "${SUDO[@]}" chmod 644 "$release_marker"; then
        info "Recorded installed Manager release ${latest_tag}."
    else
        warn "Manager was upgraded, but the release marker could not be written at ${release_marker}."
    fi
    rm -f "$marker_tmp"
...
IFS=$'\t' read -r MANAGER_RELEASE_TAG MANAGER_ASSET_API MANAGER_ASSET_DIGEST MANAGER_ASSET_SIZE MANAGER_ASSET_ID <<< "$MANAGER_INSTALL_INFO"
[[ -n "$MANAGER_RELEASE_TAG" && -n "$MANAGER_ASSET_API" && -n "$MANAGER_ASSET_ID" ]] \
    || fail "Latest Manager release metadata is incomplete."
...
MANAGER_ACTUAL_SIZE="$(stat -c '%s' "$TMP_DIR/manager.zip")"
if [[ "$MANAGER_ASSET_SIZE" =~ ^[0-9]+$ ]] && [[ "$MANAGER_ACTUAL_SIZE" != "$MANAGER_ASSET_SIZE" ]]; then
    fail "Manager release size verification failed (expected ${MANAGER_ASSET_SIZE}, received ${MANAGER_ACTUAL_SIZE})."
fi

MANAGER_ACTUAL_SHA="$(sha256sum "$TMP_DIR/manager.zip" | awk '{print $1}')"
if [[ "$MANAGER_ASSET_DIGEST" == sha256:* ]]; then
    MANAGER_EXPECTED_SHA="${MANAGER_ASSET_DIGEST#sha256:}"
    [[ "$MANAGER_ACTUAL_SHA" == "$MANAGER_EXPECTED_SHA" ]] \
...
MANAGER_PLUGIN_VERSION="$($PYTHON_BIN - "$TMP_DIR/manager/gaming-hub-manager/plugin.json" <<'PY_MANAGER_VERSION'
import json, sys
with open(sys.argv[1], encoding="utf-8") as f:
    data = json.load(f)
if data.get("id") != "gaming-hub-manager":
...
PY_MANAGER_VERSION
)" || fail "Downloaded Manager plugin manifest is invalid."
[[ -n "$MANAGER_PLUGIN_VERSION" ]] || fail "Downloaded Manager plugin manifest has no version."

mkdir -p plugins
cp -a "$TMP_DIR/manager/gaming-hub-manager" plugins/gaming-hub-manager

cat > "$MANAGER_RELEASE_MARKER" <<MARKER
TAG=${MANAGER_RELEASE_TAG}
PLUGIN_VERSION=${MANAGER_PLUGIN_VERSION}
ASSET_ID=${MANAGER_ASSET_ID}
ASSET_SHA256=${MANAGER_ACTUAL_SHA}
INSTALLED_AT=$(date -u +%Y-%m-%dT%H:%M:%SZ)
MARKER
chmod 644 "$MANAGER_RELEASE_MARKER"

info "Gaming Hub Manager ${MANAGER_RELEASE_TAG} (plugin ${MANAGER_PLUGIN_VERSION}) installed."
...
NGINX_WORKER_UID="$(
    "${DOCKER[@]}" compose exec -T nginx sh -c 'id -u nginx' 2>/dev/null \
        | tail -n 1 \
        | tr -d '[:space:]'
)"
[[ "$NGINX_WORKER_UID" =~ ^[0-9]+$ ]] \
    || fail "Could not determine the Nginx worker UID."
