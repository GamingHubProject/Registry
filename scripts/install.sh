...
INSTALLER_VERSION="2.9.1-provenance-handshake"
...
manager_prepare_verified_release() {
    ...
    if [[ "$MANAGER_ASSET_DIGEST" =~ ^sha256:([a-fA-F0-9]{64})$ ]]; then
        ...
        # Provenance uses a stable trust-source vocabulary. The parsing format is informational only.
...
complete_azuriom_custom_install() {
    [[ -d "$INSTALL_DIR" ]] || fail "No Azuriom installation exists at ${INSTALL_DIR}."
    [[ -f "$INSTALL_DIR/docker-compose.yml" ]] || fail "${INSTALL_DIR}/docker-compose.yml is missing."
    [[ -f "$INSTALL_DIR/.env" ]] || fail "${INSTALL_DIR}/.env is missing."
    ...
    printf '\nEnabling Gaming Hub Manager...\n'
    "${DOCKER[@]}" compose exec -T app php artisan plugin:enable gaming-hub-manager \
        || fail "Azuriom was initialized, but Gaming Hub Manager could not be enabled."
    ...
    # Keep the account interactive and let Azuriom validate/hash the password.
    # The password is never written to the installer credential file.
    create_admin_login

    printf '\n\033[1;32mAutomatic Gaming Hub setup complete.\033[0m\n'
    printf 'Azuriom game mode: custom\n'
    printf 'Gaming Hub Manager: enabled\n'
...
PY_ENABLED
    ...
    # Finalize the trusted file/provenance state while Manager is still disabled.
    # This prevents Manager from reconciling new files against old/missing provenance.
...
PY_AFTER
    ...
    if [[ "$post_install_ok" == "yes" ]]; then
        manager_write_provenance "$python_bin" "$provenance_path" "$plugin_dir" "$latest_version" "$directory_hash" || post_install_ok="no"
    fi

    # Only expose/re-enable the new Manager after its provenance exists atomically.
    if [[ "$post_install_ok" == "yes" && "$enabled_before" == "yes" ]]; then
        "${DOCKER[@]}" compose exec -T app php artisan plugin:enable gaming-hub-manager || post_install_ok="no"
    fi
    if [[ "$post_install_ok" == "yes" ]]; then
        "${DOCKER[@]}" compose exec -T app php artisan optimize:clear || post_install_ok="no"
    fi
    if [[ "$post_install_ok" == "yes" ]]; then
        "${DOCKER[@]}" compose exec -T app php artisan plugin:cache || post_install_ok="no"
    fi

    if [[ "$post_install_ok" != "yes" ]]; then
        warn "Upgrade finalization failed; rolling Manager files, provenance, and enabled state back together."
        # The new Manager may already have been re-enabled, so disable it before restoring old files.
        "${DOCKER[@]}" compose exec -T app php artisan plugin:disable gaming-hub-manager >/dev/null 2>&1 || true
        "${SUDO[@]}" rm -rf "$plugin_dir"
        "${SUDO[@]}" cp -a "$backup_dir/plugin" "$plugin_dir"
...
printf 'This skips the database/game-selection pages in the browser.\n\n'

read -r -p "Complete Azuriom automatically as Gaming Hub Custom Game now? [Y/n]: " AUTO_CUSTOM_SETUP
AUTO_CUSTOM_SETUP="${AUTO_CUSTOM_SETUP:-Y}"

if [[ "$AUTO_CUSTOM_SETUP" =~ ^[Yy]$ ]]; then
    complete_azuriom_custom_install
else
    printf '\nAutomatic Azuriom setup skipped.\n\n'
    ...
    printf 'After the browser installer finishes, run this installer again and choose\n'
    printf 'menu option 4 to create the first administrator login.\n\n'
fi
